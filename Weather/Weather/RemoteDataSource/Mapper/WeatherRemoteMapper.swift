...
class WeatherRemoteMapper {
    func toWeather(remoteEntity: FullWeatherRemoteResponse) -> WeatherReport {
        ...
        return WeatherReport(id: String(remoteEntity.weather[0].id), city: City(name: remoteEntity.name), main: remoteEntity.weather[0].main, description: remoteEntity.weather[0].description, temperatureReport: temperatureReport)
    }
}
